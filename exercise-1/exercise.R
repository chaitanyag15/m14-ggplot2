...
ggplot(data = diamonds.sample) + geom_point(aes(x = carat, y = price)) 

# Draw the same plot as above, but color each of the points based on their clarity.
ggplot(data = diamonds.sample) + geom_point(aes(x = carat, y = price, color = clarity)) 
...
ggplot(data = diamonds.sample) + geom_point(aes(x = carat, y = price), color = 'blue')
...
ggplot(data = diamonds.sample) + geom_line(aes(x = carat, y = price, color = carat)) 

# That's kind of messy. Try using `smooth` geometry instead.
ggplot(data = diamonds.sample) + geom_smooth(aes(x = carat, y = price, color = carat)) 


# Draw a plot with bar geometry (a bar chart), mapping the diamond's `cut` to the x-axis
ggplot(data = diamonds.sample) + geom_bar(aes(x = cut)) 
